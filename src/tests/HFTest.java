...
class HFTest extends TestDriver {
	...
	/**
	 * Size of heap file to create in test cases (65 for multiple data pages;
	 * 6500 for multiple directory pages).
	 */
	...
	protected boolean test1() {
		...
		if (status == PASS && Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames()) {
		...
		if (status == PASS) {
			...
			for (int i = 0; (i < FILE_SIZE) && (status == PASS); i++) {
				...
				if (status == PASS
						&& Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames()) {
			...
			try {
				if (f.getRecCnt() != FILE_SIZE) {
					status = FAIL;
					System.err.println("*** File reports " + f.getRecCnt() + " records, not " + FILE_SIZE + "\n");
				}
			} catch (Exception e) {
				status = FAIL;
		...
		if (status == PASS) {
			...
			while (scan.hasNext()) {
				...
				if (status == PASS && !done) {
					...
					if (len != rec.length()) {
						...
						status = FAIL;
						break;
					} else if (Minibase.BufferManager.getNumUnpinned() == Minibase.BufferManager.getNumFrames()) {
					...
					if ((rec.ival != i) || (rec.fval != i * 2.5) || (!name.equals(rec.name))) {
			...
			if (status == PASS) {
				scan.close();
				if (Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames()) {
					System.err.println("*** The heap-file scan has not unpinned " + "its page after finishing\n");
					status = FAIL;
				} else if (i != (FILE_SIZE)) {
					status = FAIL;

					System.err.println("*** Scanned " + i + " records instead of " + FILE_SIZE + "\n");
				}
			}
		}
	...
	protected boolean test2() {
		...
		if (status == PASS && Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames()) {
			...
			System.out.println(
					"\nt2: in if: Number of unpinned buffers: " + Minibase.BufferManager.getNumUnpinned() + "\n");
			System.err.println("t2: in if: getNumbfrs: " + Minibase.BufferManager.getNumFrames() + "\n");
	...
	protected boolean test3() {
		...
		if (status == PASS && Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames()) {
			...
			System.out.println("t3, Number of unpinned buffers: " + Minibase.BufferManager.getNumUnpinned() + "\n");
			System.err.println("t3, getNumbfrs: " + Minibase.BufferManager.getNumFrames() + "\n");
		...
		if (status == PASS) {
			...
			while (scan.hasNext()) {
				...
				if (!done && status == PASS) {
					...
					if ((rec.ival != i) || (rec.fval != (float) i * 7) || (rec2.ival != i) || (rec2.fval != i * 7)) {
