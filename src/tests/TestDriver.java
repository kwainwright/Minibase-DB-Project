...
/**
 * <h3>Minibase Test Driver</h3> This base class contains common code to each
 * layer's test suite.
 */
class TestDriver implements GlobalConst {
	...
	/**
	 * Incremental history of the performance counters; odd elements are
	 * snapshots before the tests, and even elements are after.
	 */
	...
	protected void printCounters() {
		...
		System.out.println();
		Minibase.BufferManager.flushAllPages();
		System.out.println("  *** Number of reads:  " + (Minibase.DiskManager.getReadCount() - data.reads));
		System.out.println("  *** Number of writes: " + (Minibase.DiskManager.getWriteCount() - data.writes));
		System.out.println("  *** Net total pages:  " + (Minibase.DiskManager.getAllocCount() - data.allocs));
		int numbufs = Minibase.BufferManager.getNumFrames();
		System.out.println(
				"  *** Remaining Pinned: " + (numbufs - Minibase.BufferManager.getNumUnpinned()) + " / " + numbufs);
